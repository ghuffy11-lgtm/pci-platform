...
| ↳ | **EPA-0005 was ACCEPTED 2026-08-22 by MSG-0092, and the row above is additive-corrected here rather than rewritten.** That row still describes EPA-0005 as `PROPOSED`, which was true when written; **MSG-0092 accepted it as the architecture evaluation record *and* as the ruling record for the runtime seam**. What MSG-0092 settled: **Approach C is chosen** — a governed application layer plus a **separate document/inference worker behind an explicit contract**, where **the worker is not an authorization authority**; EPA-0005 §9.1's **three constraints become settled architecture constraints**; and **no generic stack ADR is to be created**. **EPA-0005 was not promoted to `docs/`** — MSG-0092 did not authorize promotion, and promotion is the Lead's act — so it stays in `implementation/architecture/` with its authority resting on MSG-0092. **MSG-0092 §4 keeps every provider, framework, model, engine and runtime selection deliberately open** | — | — | — |
| ↳ | **A SECOND, DISTINCT A-STACK EXERCISE RAN 2026-08-23 as TASK-0032 (MSG-0098 → MSG-0100). It is not a re-run of TASK-0026, and the row above is NOT superseded by it.** Both are labelled "A-STACK" because §6.2 lists the task once; **they answer different questions.** TASK-0026 evaluated the stack **shape** (Approaches A/B/C) against the ADR set as it stood on 2026-08-22. **TASK-0032 evaluated technology *classes* within the settled Approach C, and against ADR-0020 as amended by AMD-01** — **neither of which existed when EPA-0005 was written**: Approach C was chosen by MSG-0092 *after* EPA-0005 was delivered, and AMD-01 was accepted by MSG-0095 and applied in place by TASK-0031 on the morning of 2026-08-23. Delivered **`EPA-0006`** in `implementation/architecture/`, **PROPOSED — and it selects nothing.** It derives the **four-part predicate an engine must express in-query** (scope equality; multi-valued audience/classification overlap; **a two-sided temporal range with an open upper bound**, from ADR-0018 §4; and exclusion of SUPERSEDED, the one lifecycle state that is a query predicate rather than an ingestion invariant); sorts candidates into **three execution shapes** and applies the AMD-01 disqualifier to **seven engine classes** with reasoning — **post-filter-only and hosted/managed retrieval are DISQUALIFIED** (the latter twice over and independently, by ADR-0022 §1 as well); and specifies a **three-tier conformance probe**. **Its sharpest finding: AMD-01's G3 evidence rule does not discharge AMD-01's engine-selection criterion** — a query-shape check cannot distinguish a conforming engine from one that over-fetches internally, so engine clearance needs execution evidence of its own, and **an engine exposing no plan or counter instrumentation cannot be cleared at all**. **Every product-level selection remains OPEN**, **no benchmark, capacity, latency or corpus-scale figure appears anywhere in the record**, **ADR-0019 is untouched and no normalization rule was written**, **no ADR was created, amended, or proposed**, and **no implementation task was marked READY** | — | — | — |
| ↳ | **EPA-0006's three-tier conformance probe WAS RUN, 2026-08-23 as TASK-0033 (MSG-0101 → MSG-0104), and it CLEARED NOTHING.** The row above specified the probe; this row records executing it. **One engine was reachable** — **SQLite 3.51.3 embedded in the Node runtime**, a class-**R** member, exercised as a **test subject only** (MSG-0101 §3) in three query shapes plus a **deliberately non-conforming negative control**, against a synthetic in-memory fixture: **24 candidate executions across 6 fixtures** (2 index designs × collection sizes M ∈ {50, 500, 5000}), all three tiers. **Tier 1 PASSED** — all four predicate constraints, including the open-ended temporal range and the multi-valued set overlap, are expressible in one query. **Tier 2 PASSED** — k-complete and result-set-invariant as M grows — **and the negative control FAILED it**, returning correct results at M=50 and **nothing at all** at M=500 and M=5000, so a post-filter design **looks correct on a small collection and degrades into empty answers as it grows**. **Tier 3 decided the outcome, against the candidate:** every tested shape makes the engine examine **unauthorized rows in a number that grows linearly with the collection** (3000 of 5000 with a partial index; **1000 of 5000 even with the widest index**), because the **multi-valued audience conjunct cannot be pushed into the index** — while returning results **identical to a conforming engine's**. **EPA-0006 findings 1 and 2 are now demonstrated experimentally**: the same SQL and the same engine, with the plan forced away from the authorization index, reached 5008 rows instead of 3008 and **returned the identical correct answer**. **Verdict: NOT CLEARED for the one engine probed; class D DISQUALIFIED and now demonstrated; classes S, V and K NOT CLEARED with zero execution evidence** (no Docker CLI reachable, no PostgreSQL). **Nothing selected, adopted, installed or deployed; no accepted ADR modified; no implementation task authorized; no `EPA-0007` created; no benchmark or timing figure recorded.** One non-blocking clarification is referred: whether AMD-01's Shape-1 bar means *"examines nothing unauthorized"* — which may be unsatisfiable by any engine — or *"materializes no unauthorized content"*. Record: **MSG-0104** | — | — | — |
