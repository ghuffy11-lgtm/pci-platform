...
| TASK-0048 | **Bounded N6 measurement — measure byte-level durability containment against EPA-0006 §4.18** | **READY** | **MSG-0161** (Q20 = **YES**) AUTHORIZED; the Lead's committed task definition [`TASK-0048-n6-measurement.md`](TASK-0048-n6-measurement.md) (`fef8bad`); **MSG-0160** (Q19 = YES); TASK-0047 COMPLETE; **EPA-0006 §4.18** (N6, N6.1–N6.3), **§4.13** (N1–N5, W1–W4) and **§4.16** (DA-1…DA-7) binding as written | — **not yet executed** | Execute the bounded N6 measurement exactly as the Lead's task file defines it. **Scope is fixed by MSG-0161 Q20: the four MSG-0158 physical organizations — L1 shared projection, L2 isolated structures in one store, L3 isolated stores, L4 isolated stores after re-partition — with both journal modes and both request-induced write shapes (W-A access accounting, W-B cache writeback) where applicable.** **Where the task file and this row differ, the FILE WINS and the difference is reported** — the TASK-0045/TASK-0046 rule. **Evidence-only: it may establish N6 findings or NOT CLEARED, and it may not select, adopt, deploy, implement or clear an engine, change N1–N5, DA-1…DA-7, E1–E4, strict Shape-1 or any clearance gate, or move a candidate verdict.** **Unmeasured is not satisfied** — no topology holds an N6 status today. **Do not silently re-run TASK-0046 and report its output as new N6 evidence.** **Negative controls must fire or the run is INVALID** (MSG-0156: a control's finding may never be reported as a finding about the subject). **Stop if `origin/main` moves mid-run** | Claude Code |
