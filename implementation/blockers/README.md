...
| [BLK-0014](BLK-0014-third-binding-present-and-not-executable-by-this-runner.md) | **A THIRD SQLite binding is INSTALLED on this host and this runner may not execute it** | **Bounds the survey — and it is CHEAP to clear.** **This is NOT an install request**: the artefact is already on the disk | **OPEN** 2026-08-26 — **TASK-0054 executed and answered its question**; this records one thing it found and could not measure. **`bun` is installed at `C:\Users\Administrator\.bun\bin\bun.exe` (98 480 216 bytes, mtime 2026-05-12) and ships `bun:sqlite` — a THIRD BINDING**, which is **the variable EPA-0006 §4.15 identified as decisive**: *"the two subjects differ in the BINDING, not in the build."* **Twelve prior probes ran against TWO bindings; the third has been on this machine the whole time and no probe has ever enumerated it.** **Found only because the enumeration was deliberately WIDENED past a fixed name list** — all 29 PATH directories scanned by engine vocabulary with a fabricated-name negative control, plus runtime install directories read directly (DISC-0014's lesson applied). **`bun --version` attempted ONCE and refused; cause established by READING both permission files rather than inferred from the failure — neither `.claude/settings.local.json` nor `runner-settings.json` carries ANY `bun` entry.** **A bare command failure is not a diagnosis: this is a PERMISSION boundary, not an install boundary, and nobody should be sent to install bun.** **No workaround taken** — no `child_process`, no shell indirection, no alternate path (BLK-0011's precedent followed exactly). **Option A is ONE LINE in a version-controlled permission file**, granting no write and no network capability, on **BLK-0010 / BLK-0011's option-A shape**. **The five MSG-0173b §1 items are carried, and item 4 is answered honestly in BOTH directions: it would establish whether `bun:sqlite` exposes a statement surface that accumulates, can be read back, and offers unexpanded text — and it would NOT discharge GAP-B** (MSG-0180 §4: finding a surface *"would require a separate authorized measurement"*), **would NOT clear any candidate, is STILL SQLite, and the LIKELY OUTCOME IS ANOTHER C1 = NO. The operator should grant it only because a cheap negative is worth having, not because it is expected to succeed.** **Seven cheaper alternatives were checked FIRST and are recorded with their results**, which is why the ask is a permission line rather than an install — including **ODBC driver-manager tracing, rejected on TWO independent grounds** (a registry/host change, **and** the driver manager's log rather than the engine's), and **Windows ESE/JET Blue, rejected on its own semantics: it has no SQL statements, so it cannot supply a statement log.** **Option B — decline, and restate the standing finding honestly as "no log exists in the bindings this programme can reach", naming the third binding as ENUMERATED AND UNMEASURED**, so no later reader mistakes the claim's width. **Option A costs almost nothing and decides whether Option B's restatement is the right one — recommended first on BLK-0012's own reasoning.** **Does NOT supersede BLK-0012**: that records an unsatisfiable criterion, this an unmeasured binding. **Evidence: MSG-0181** |
| [BLK-0013](BLK-0013-task-0050-push-rejected-origin-moved-mid-run.md) | **`origin/main` moved mid-run; TASK-0050's records are committed locally and cannot be published** | **Communication channel down for that session** — the evidence work is complete and **none of it is on `main`** | **CLEARED** 2026-08-26 — **the operator reconciled the divergence and pushed (`f9f8f07..dd99f37`); the records are on `origin/main` and were verified there by the Lead in MSG-0169 §3.** TASK-0050's outcome 7 is discharged and the task is **COMPLETE — 7/7**. **The cause is now known and is NOT what this record guessed:** the Lead's own hourly Routine pushed to `main` mid-run, which MSG-0169 §4 records as **caused by the Lead**, and the two-writer condition it created is ruled on in **MSG-0170** (the Lead writes to a branch whenever a task is READY). **Index row corrected by MSG-0182 §6** — it read OPEN while the blocker's own record read CLEARED. *Originally recorded as:* **OPEN** 2026-08-26 — `git push origin main` returned **`! [rejected] main -> main (fetch first)`**, so the remote holds work this session does not have. **This is CLAUDE.md's fail-closed *"mid-run repository movement — abort"* boundary** (MSG-0028 decision 2), not an error to retry past. **No force-push, rebase, merge, pull or reset**; three commits (`f41c202`, `f063f09`, `339157f`) intact and unpublished. **No reconciliation is possible from this session**: `git fetch`, `git fetch --all`, `git ls-remote` and `git push --dry-run` were each tried **once** and each returned *"This command requires approval"* — **cause established by reading both permission files**, which grant `git push origin main` and **no read of the remote at all**. **The transport is FINE and this corrects a diagnosis that could have been wrong**: the push **reached** `github-pci` and was refused at protocol level, so this is **not** BLK-0007's transport fault and **nobody should be sent to fix SSH**. **What moved it is UNKNOWN and deliberately not guessed** — MSG-0166's hourly Lead Routine is offered as the first place to look, **as an inference, not an observation**. **Consequence: TASK-0050's outcome 7 is UNMET and the task is BLOCKED, not COMPLETE.** **Option A — reconcile in an attended session; option B — grant the runner a read-only `git fetch`, which removes the whole class of failure and gives no write capability.** **They are not alternatives** |

**Two blockers are open: BLK-0012 and BLK-0014.** **They are unrelated in cause and should not be read
together.** **BLK-0012 is about the criterion** — no reachable subject supplies E4 both obtainable and
non-adverse, which is the reach the GAP-B answer is bounded by. **BLK-0014 is about an instrument** — a
third SQLite binding is installed on this host, has never been enumerated, and the runner may not
execute it. **BLK-0014 does not supersede BLK-0012**, and clearing BLK-0014 would not discharge GAP-B.

> **Superseded, kept because the correction is worth more than a tidy record.** This paragraph
> previously read *"Two blockers are open: BLK-0012 and BLK-0013 … BLK-0013 must be cleared first, or
> nobody but this machine can read the work that BLK-0012 describes."* **BLK-0013 was CLEARED on
> 2026-08-26** when the operator reconciled the divergence and pushed (MSG-0169 §3), and **BLK-0014 was
> raised on the same day by the TASK-0054 runner.** The index row was stale while the blocker's own
> record was correct; both were corrected by **MSG-0182 §6**, which is also where the standing rule
> sits: **when an index and a record disagree, fix the index and say so — never quietly.**
