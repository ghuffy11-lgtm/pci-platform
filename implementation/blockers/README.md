...
| [BLK-0008](BLK-0008-designated-corpus-unreachable.md) | Designated A-SURVEY corpus (NFS export `\\10.1.27.220\LXBackup\plan.pdf`) is not reachable | Medium | **OPEN** 2026-08-22 — **corrected: the path is NFS, not SMB.** Two independent blockers: **NFS 2049 and portmapper 111 are both closed**, *and* **Client for NFS is not installed** on this workstation (`NFS-Client` = `Available`). Not a credentials problem — no TCP connection, so no authentication is attempted. Installing the feature is a privileged change nothing authorizes. A-SURVEY stays unexecutable; A-STACK unaffected |
