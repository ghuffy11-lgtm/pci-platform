...
|---|---|---|---|
| **MSG-0072** | **ADR promotion gap after MSG-0071** | **OPEN** — **decision needed.** MSG-0071 accepted **all six** ADRs and directed promotion, but only **ADR-0017** is in `docs/decisions/`; ADR-0018…0022 remain drafts. Indexes and drafts reconciled to record the acceptance; **promotion itself was NOT performed** — it is the act that confers authority and no READY task authorizes it. **Pre-promotion verification passed:** no provider/model/runtime selection leaked in, ADR-0019 invents no normalization rules, ADR numbering collision-free | [MSG-0072-adr-promotion-gap.md](MSG-0072-adr-promotion-gap.md) |
...
| **MSG-0069** | **TASK-0024 queue reconciliation — seventh recurrence, doubled collision** | **OPEN** — informational. TASK-0024 reconciled as the **single READY task**; the Supervisor was observed idling at `NOOP` while it sat authorized. **Two spec files and two authorization messages** for one task, all four in agreement; queue section carries the **union** | [MSG-0069-task-0024-queue-reconciliation.md](MSG-0069-task-0024-queue-reconciliation.md) |
...
| **MSG-0065** | **Correction: Schedule service running, scheduled task disabled** | **OPEN** — informational, but it changes the operator action. Records said the Windows `Schedule` service was stopped; **verified: the service is Running (Automatic) and the task `PCI-Execution-Supervisor` is Disabled**. Same observable effect (no cycle fires) but a different remedy — **enable the task**, not restart the service. Task not enabled; state left as found | [MSG-0065-supervisor-schedule-state-correction.md](MSG-0065-supervisor-schedule-state-correction.md) |
| **MSG-0064** | **TASK-0023 queue reconciliation** | **OPEN** — informational. TASK-0023 reconciled into the queue as the **single READY task** after prerequisites were verified individually; the **sixth** recurrence of an authorization existing while the queue did not reflect it. Verified by dry run; **not executed in the same session**, per the operator instruction | [MSG-0064-task-0023-queue-reconciliation.md](MSG-0064-task-0023-queue-reconciliation.md) |
